...
abstract class SearchEngineIndexation
{
    ...
    {
        $result = SearchEngineDb::query('SELECT acp.`id_cache_product`, pa.`id_product_attribute`, pa.`quantity`, pa.`id_product`
    ...
    protected function getProductWeight(Product $product)
    {
    ...
    {
        return array(
                array('name'=> Module::getInstanceByName(_PM_AS_MODULE_NAME_)->translateMultiple('yes'),'value'=>1),
                array('name'=> Module::getInstanceByName(_PM_AS_MODULE_NAME_)->translateMultiple('no') ,'value'=>0),
        );
    }
    public static function getConditionCriteria($value = false)
    {
        $return = array(
                'new'=>array('name'=> Module::getInstanceByName(_PM_AS_MODULE_NAME_)->translateMultiple('new'),'value'=>'new'),
                'used'=>array('name'=> Module::getInstanceByName(_PM_AS_MODULE_NAME_)->translateMultiple('used') ,'value'=>'used'),
                'refurbished'=>array('name'=> Module::getInstanceByName(_PM_AS_MODULE_NAME_)->translateMultiple('refurbished') ,'value'=>'refurbished')
        );
        if ($value) {
    ...
    public static function getProductsIdFromProductField($id_search, $field_value, $field)
    {
        ...
        if ($field == 'stock') {
            ...
            $where_clause = 'p.`id_product` IN (' . implode(',', array_map('intval', $idProductListSubscription)) . ')';
    ...
    public static function setProductsPrices($id_search, $id_product = false)
    {
        ...
        while ($nb_products_specific_price > 0) {
            foreach ($products_price as $row) {
                ...
                if ($id_shop == 0) {
                    $liste_id_shop = array(SearchEngineUtils::getShopBySearch($id_search));
                } else {
        ...
        foreach ($advanced_searchs_id as $id_search) {
            ...
            foreach ($criterions_groups_indexed as $row2) {
                if ($row2['criterion_group_type'] == 'manufacturer' && $product->id_manufacturer) {
                    ...
                    self::indexCriterionsGroup($id_search, $row2['criterion_group_type'], 0, $row2['id_criterion_group'], $row2['visible'], false, true, $product->id, false, (int)$product->on_sale);
                } elseif ($row2['criterion_group_type'] == 'available_for_order' && $product->available_for_order) {
                    self::indexCriterionsGroup($id_search, $row2['criterion_group_type'], 0, $row2['id_criterion_group'], $row2['visible'], false, true, $product->id, false, (int)$product->available_for_order);
                } elseif ($row2['criterion_group_type'] == 'online_only' && $product->online_only) {
                    self::indexCriterionsGroup($id_search, $row2['criterion_group_type'], 0, $row2['id_criterion_group'], $row2['visible'], false, true, $product->id, false, (int)$product->online_only);
                    ...
                    self::indexCriterionsGroup($id_search, $row2['criterion_group_type'], 0, $row2['id_criterion_group'], $row2['visible'], false, true, $product->id, false, ($globalStockAvailable > 0));
                } elseif ($row2['criterion_group_type'] == 'weight') {
                    self::indexCriterionsGroup($id_search, $row2['criterion_group_type'], 0, $row2['id_criterion_group'], $row2['visible'], false, true, $product->id, false, self::getProductWeight($product));
                    ...
                    self::indexCriterionsGroup($id_search, $row2['criterion_group_type'], 0, $row2['id_criterion_group'], $row2['visible'], false, true, $product->id, false, (int)\AdvancedPack::isValidPack($product->id));
                } elseif ($row2['criterion_group_type'] == 'subscription') {
                    self::indexCriterionsGroup($id_search, $row2['criterion_group_type'], 0, $row2['id_criterion_group'], $row2['visible'], false, true, $product->id, false, (int)\PmSubscription\Models\Product::subscriptionAllowed($product->id));
                } elseif ($row2['criterion_group_type'] == 'new_products' && $product->new) {
                    self::indexCriterionsGroup($id_search, $row2['criterion_group_type'], 0, $row2['id_criterion_group'], $row2['visible'], false, true, $product->id, false, (int)$product->new);
                }
            }
        }
        return true;
    ...
    public static function indexCriterionsGroup($id_search, $criterions_group_type, $id_criterion_group_linked, $id_criterion_group = false, $visible = 1, $checkIfIsIndexed = true, $update = false, $id_product = false, $id_criterion_linked = false, $criterion_value = false)
    {
        ...
        if ($update) {
            ...
            if ($criterions_group_type == 'price') {
                self::deleteCachePriceGroup($objSearch->id, $id_product);
            }
        ...
        if ($criterions_group_type == 'attribute') {
            ...
            $criterions_group = array(
                'name'    => Module::getInstanceByName(_PM_AS_MODULE_NAME_)->translateMultiple('categories', $id_criterion_group_linked),
            );
        } else {
            $criterions_group = array(
                    'name'    => Module::getInstanceByName(_PM_AS_MODULE_NAME_)->translateMultiple($criterions_group_type),
            );
        }
        ...
        $objAdvancedSearchCriterionGroupClass = new CriterionGroup($id_criterion_group, $objSearch->id);
        if (!$update) {
        ...
        self::$processingIndexation = false;
        $objAdvancedSearchCriterionClass = new Criterion(false, $objSearch->id);
    ...
    public static function reindexAllSearchs($fromCron = false)
    {
        ...
        if ($fromCron) {
            $advanced_searchs_id = SearchEngineUtils::getSearchsId(false, Context::getContext()->shop->id);
        } else {
            $advanced_searchs_id = SearchEngineUtils::getSearchsId(false);
        }
        foreach ($advanced_searchs_id as $idSearch) {
